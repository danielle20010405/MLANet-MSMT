...
def calc_nme_with_image_diag(pts68_pred, pts68_true, img_path):
    """Calculate NME normalized by image diagonal length."""
    # 加载图像获取宽度和高度
    img = Image.open(img_path)
    width, height = img.size
    llength = sqrt(width ** 2 + height ** 2)  # 图像对角线长度

    # 打印图像尺寸和对角线长度以进行验证
    # print(f"Image: {img_path}, Width: {width}, Height: {height}, Diagonal: {llength}")
...
def calc_nme_for_all_with_image_diag(pred_lms, gt_lms, img_folder, files):
    """Calculate NME for all predicted and ground truth landmarks normalized by image diagonal length."""
    nme_list = []
    for pred_lm, gt_lm, file in zip(pred_lms, gt_lms, files):
        img_path = osp.join(img_folder, file.replace('.txt', '.jpg').replace('.mat', '.jpg'))
        if osp.exists(img_path):
            nme = calc_nme_with_image_diag(pred_lm, gt_lm, img_path)
            nme_list.append(nme)
        else:
...
# 设置路径
pred_folder = '../../../checkpoints/deep3d/co-nodownaff/results/AFLW2000'
...
print("First set of landmarks:")
print("Predicted landmarks (first set):")
print(pred_lms[0])
print("Ground truth landmarks (first set):")
print(gt_lms[0])
...
# 计算 NME
nme_list = calc_nme_for_all_with_image_diag(pred_lms, gt_lms, img_folder, pred_files)
