...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.fusion = AFF(self.inplanes)

        self.conv_downsample_1 = nn.Conv2d(256, 512, kernel_size=1, stride=2)
        self.conv_downsample_2 = nn.Conv2d(512, 1024, kernel_size=1, stride=2)
        self.conv_downsample_3 = nn.Conv2d(1024, 2048, kernel_size=1, stride=2)
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        x1 = self.layer1(x)
        x1 = self.layer_atten[0](x1)

        x2 = self.layer2(x1)
        x2 = self.layer_atten[1](x2)

        x3 = self.layer3(x2)
        x3 = self.layer_atten[2](x3)

        x4 = self.layer4(x3)
        x4 = self.layer_atten[3](x4)
    

        # Apply AFF between layers

        x4_up = self.up(x4)
        x3 = self.fusion(x4_up, self.channel_change[2](x3))

        x3_up = self.up(x3)  
        x2 = self.fusion(x3_up, self.channel_change[1](x2))  

        x2_up = self.up(x2)  
        x1 = self.fusion(x2_up, self.channel_change[0](x1))
