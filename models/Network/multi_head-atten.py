...
from .attention import CoordAtt, MS_CAM, AFF
...
class ReconNetWrapper(nn.Module):
    ...
    def forward(self, x):
        ...
        # if not self.use_last_fc:
        #     output = [layer(x) for layer in self.final_layers]  # Apply each final layer
        #     x = torch.cat(output, dim=1)  # Concatenate outputs
        #     x = torch.flatten(x, 1)  # Flatten to (batch_size, total_output_dim)
        return x
...
class AttentionPool2d(nn.Module):
    ...
    def forward(self, x):
        x = x.reshape(x.shape[0], x.shape[1], x.shape[2]
                      * x.shape[3]).permute(2, 0, 1)  # (B, C, H, W) → (H*W, B, C)
        x = torch.cat([x.mean(dim=0, keepdim=True), x], dim=0)  # (HW*W, B, C) → (HW+1, B, C)
...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        # 新增 AttentionPool2d 代替 avgpool

        self.attention_pool = AttentionPool2d(spacial_dim=7, embed_dim=2048, num_heads=8, output_dim=2048)

        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))

        self.downsample = nn.ModuleList([nn.Conv2d(256,512,kernel_size=1, stride=2),
                                         nn.Conv2d(512,1024,kernel_size=1, stride=2),
                                         nn.Conv2d(1024,2048,kernel_size=1, stride=2)])

        self.up = nn.Upsample(scale_factor=2, mode='bilinear')
        
        self.channel_change = nn.ModuleList([nn.Conv2d(256, 2048, kernel_size=1), 
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        for i in range(len(self.fuse_layers)):
            ...
            y_list[i] = self.relu(y)  # Apply ReLU after fusion

        # The fused output can now be further processed, for example by averaging:
        x = y_list[-1]  # Use the last fused layer (most reduced scale)
        print(x.shape)
