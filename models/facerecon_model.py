...
class FaceReconModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--net_recon', type=str, default='resnet50', choices=['resnet18', 'resnet34', 'resnet50', 'resnext50_32x4d'], help='network structure')
        parser.add_argument('--init_path', type=str, default='checkpoints/init_model/resnet50-0676ba61.pth', choices=['preresnext50_32x4d.pth', 'resnet50-0676ba61.pth'])
        ...
        if is_train:
            ...
            # loss weights
            parser.add_argument('--w_feat', type=float, default=0.35, help='weight for feat loss') #0.2
            parser.add_argument('--w_color', type=float, default=1.5, help='weight for loss loss') #1.92
            parser.add_argument('--w_reg', type=float, default=5.0e-4, help='weight for reg loss') #3.0e-4
            parser.add_argument('--w_id', type=float, default=1.3, help='weight for id_reg loss') #1.0
            parser.add_argument('--w_exp', type=float, default=1.1, help='weight for exp_reg loss') #0.8
            parser.add_argument('--w_tex', type=float, default=0.06, help='weight for tex_reg loss') #1.7e-2
            parser.add_argument('--w_gamma', type=float, default=5.0, help='weight for gamma loss') #10.0
            parser.add_argument('--w_lm', type=float, default=0.01, help='weight for lm loss') #1.6e-3
            parser.add_argument('--w_reflc', type=float, default=3.0, help='weight for reflc loss') #5.0
    ...
    def compute_visuals(self):
        with torch.no_grad():
            ...
            if self.gt_lm is not None:
                ...
                print("gt_lm_numpy:")
                output_vis_numpy_gt = util.draw_landmarks(output_vis_numpy_raw, gt_lm_numpy, 'b')
                print("pred_lm_numpy:")
                output_vis_numpy_pre = util.draw_landmarks(output_vis_numpy_raw, pred_lm_numpy, 'r')
                output_vis_numpy_all = util.draw_landmarks(output_vis_numpy_gt, pred_lm_numpy, 'r')
            
                output_vis_numpy = np.concatenate((input_img_numpy, 
                                    output_vis_numpy_raw, 
                                    output_vis_numpy_gt, 
                                    output_vis_numpy_pre, output_vis_numpy_all), axis=-2)
            else:
                output_vis_numpy = np.concatenate((input_img_numpy, 
