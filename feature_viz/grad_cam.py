...
def process_image(model, img_path, target_layers, data_transform, output_folder):
    # 加载图像
    ...
    for i, fmap in enumerate(feature_maps):
        ...
        # 保存叠加后的输出图像
        output_img_path = os.path.join(output_folder, f"{os.path.splitext(os.path.basename(img_path))[0]}_feature_map_layer_{i}.jpg")
...
def main():
    ...
    target_layers = [model.backbone.layer4[-1], model.backbone.fusion[2]]
    ...
    # 指定图像文件夹路径
    img_folder = "../../dataset/deep3d/AFLW2000-3D/img/example_train"
    assert os.path.exists(img_folder), f"folder: '{img_folder}' does not exist."

    output_folder = "../../dataset/deep3d/AFLW2000-3D/grad_output/"
    os.makedirs(output_folder, exist_ok=True)

    img_files = [f for f in os.listdir(img_folder) if os.path.isfile(os.path.join(img_folder, f)) and f.lower().endswith(('.jpg', '.png', '.jpeg'))]

    selected_img_files = random.sample(img_files, min(500, len(img_files)))

    # 遍历文件夹中的所有图像文件
    for img_file in selected_img_files:
        img_path = os.path.join(img_folder, img_file)
        process_image(model, img_path, target_layers, data_transform, output_folder)
